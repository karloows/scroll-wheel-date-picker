...
- Never run `dart pub publish` / `flutter pub publish` yourself — publishing to pub.dev is automated: merging the `release-please` PR triggers the `publish` job in `.github/workflows/release-please.yml`, which runs it via pub.dev's OIDC trusted publishing (no token stored in the repo).
